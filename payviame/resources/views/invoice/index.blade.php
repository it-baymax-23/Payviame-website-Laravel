    ...
    <div class="page-links">
      <div class="container">
        <div class="left-penal">   
          <div id="new-in">  
            <div id="flip"><button class="flip-in">New Invoice</button></div>
            <div id="panel">
              <form class="dropdown_inline_form" method="POST" action="{{route('invoice.store')}}">
                @csrf
                <span style="color: #fff; font-size: 18px;"> New Invoice </span>
                &nbsp;&nbsp;&nbsp;
          ...
          <!-- <div>
            <button id="new-creat" class="flip-in" data-toggle="modal" data-target="#myModal" style="display: none;">Create a Recurring Invoice</button>
          </div> -->
        </div>
      ...
      <div id="home" class="container tab-pane invoice-tab-1 active  ">
        <div class="mb-3 mt-5">
          <select id="status" name="status" class="custom-select" style="width:170px;float: left; margin-right: 10px;">
            <option value="unpaid" <?php echo ($status == 'unpaid') ? 'selected' : '' ?>>Unpaid Invoices</option>
            <option value="overdue" <?php echo ($status == 'overdue') ? 'selected' : '' ?>>Overdue Invoices</option>
            <option value="paid" <?php echo ($status == 'paid') ? 'selected' : '' ?>>Paid Invoices</option>
          ...
          <select id="by" name="by" class="custom-select" style="width:120px;float: left; margin-right: 10px;">
            ...
            <option value="due_at" <?php echo ($by == 'due_at') ? 'selected' : '' ?>>Due</option>
            <option value="paid_at" <?php echo ($by == 'paid_at') ? 'selected' : '' ?>>Paid</option>
          </select>
        ...
        <div class="summary-row mb-3 mt-4">
          ...
          <div class="row">
            <div class="col-3">
              <div class="summary-heading">PAID</div>
              <div class="currency-summary">
                {{config('app.currency_symbol')}} {{$paid}}<small class="grey"> {{config('app.currency_code')}}</small></div>
            </div>
            <div class="col-3">
              <div class="summary-heading">OWING</div>
              <div class="currency-summary">{{config('app.currency_symbol')}} {{$owing}}<small class="grey"> {{config('app.currency_code')}}</small></div>
            </div>
            <div class="col-3">
              <div class="summary-heading">OVERDUE</div>
              <div class="currency-summary">{{config('app.currency_symbol')}} {{$overdue}}<small class="grey"> {{config('app.currency_code')}}</small></div>
            </div>
            <div class="col-3">
          ...
          <div class="row">
            <div class="col-3">
              <div class="summary-heading">PAID</div>
              <div class="currency-summary">
                {{$current_currency->currency_symbol}} {{$paid}}<small class="grey"> {{$current_currency->currency_code}}</small></div>
            </div>
            <div class="col-3">
              <div class="summary-heading">OWING</div>
              <div class="currency-summary">{{$current_currency->currency_symbol}} {{$owing}}<small class="grey"> {{$current_currency->currency_code}}</small></div>
            </div>
            <div class="col-3">
              <div class="summary-heading">OVERDUE</div>
              <div class="currency-summary">{{$current_currency->currency_symbol}} {{$overdue}}<small class="grey"> {{$current_currency->currency_code}}</small></div>
            </div>
            <div class="col-3">
              <div class="summary-heading">TOTAL</div>
        ...
        <div class="mb-3 mt-4">
          ...
          <div class="grid invoice">
            <div class="invoice_client_details">
              @if(!isset($invoice->client->client_logo))
              <div class="badge " style="background-color: {{ $invoice->client->monogram_color }}">{{ $invoice->client->monogram_name}}</div>
              @else
              <img src="{{asset($invoice->client->client_logo)}}" width="40" height="40">
              @endif
              <span class="number">{{ $invoice->id }}</span>
            </div>
            <!-- <div class="btn-group invoice-actions">
              ...
              <ul class="dropdown-menu pull-right">
                <li>
                  <a target="_blank" href="/invoices/759246/print"><i class="fa fa-print"></i>&nbsp;&nbsp; Print</a>
                </li>
                <li>
                  <a target="_blank" href="/invoices/759246.pdf"><i class="fa fa-download"></i>&nbsp;&nbsp; Get PDF</a>
                </li>
                <li>
                  <a target="_blank" href="/time?invoice=759246&amp;range=all-time"><i class="fa fa-history"></i>&nbsp;&nbsp; Billed Time</a>
                </li>
                <li>
                  <a target="_blank" href="/invoices/759246/view/g3rgg2vpv0nbk8um9gf2tke01wtpqut0"><i class="fa fa-globe"></i>&nbsp;&nbsp; View Online</a>
                </li>
              </ul>
            </div> -->
            @if($invoice->status == 0)
            ...
            <div class="total">
              <span class="currency">{{$invoice->currency->currency_symbol}}</span><span class="amount">{{ $invoice->sum_total }}</span>
            </div>
            <div class="currency_code">{{$invoice->currency->currency_code}}</div>
...
@push('scripts')
  ...
  <script>
    ...
    //   if (currency_id == 0)
    //     currency_id = '<?php echo $user->profile->currency_id?>';
    //   url = '{{config('app.url')}}' + '/' + client_id + '/invoices/' + currency_id;
    ...
    $('#status').change(function(){
      ...
      url = '{{config('app.url')}}' + '/invoices?start=' + start + '&end=' + end + '&status=' + status + '&range=' + range + '&by=' + by;
      ...
      if (client_id != 0 || currency_id != 0) {
        if (client_id != 0) {
          if (currency_id != 0) {
            url = '{{config('app.url')}}' + '/invoices?start=' + start + '&end=' + end + '&status=' + status + '&range=' + range + '&by=' + by + '&client_id=' + client_id + '&currency_id=' + currency_id;
            // console.log(url);
          } else {
            url = '{{config('app.url')}}' + '/invoices?start=' + start + '&end=' + end + '&status=' + status + '&range=' + range + '&by=' + by + '&client_id=' + client_id;
            // console.log(url);
          }
        } else {
          if (currency_id != 0) {
            url = '{{config('app.url')}}' + '/invoices?start=' + start + '&end=' + end + '&status=' + status + '&range=' + range + '&by=' + by + '&currency_id=' + currency_id;
            // console.log(url);
          }
        }
      }
